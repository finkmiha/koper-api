...
const ExportDAO = require('../dao/export');
...
/**
 * Export user work for selected month.
 *
 * @param {string} month Month format "YYYY-MM".
 */
async function exportMyWork(ctx, next) {
	...
	// 		}
	// 	} else if (type == 'Extra hours' || type == 'Effective work') {
	// 		work[day] = {
	// 			work_start: [start],
	// 			work_end: [end],
	...
	// let spreadsheets = [{ name: user.name,
	// 	data: data }];
	...
	await ExportDAO.createExcelExport(ctx, body.month);
